...
CREATE TABLE Dancivnyk (
    ...
    imya VARCHAR(100) NOT NULL CHECK (imya ~ '^[A-Z][a-z\\s]+$'),
...
ALTER TABLE Dancivnyk ADD CONSTRAINT dancivnyk_imya_format
CHECK (imya ~ '^[A-Z][a-z\\s]+$');

ALTER TABLE Psyholog ADD CONSTRAINT psyholog_imya_format
CHECK (imya ~ '^[A-Z][a-z\\s]+$');
